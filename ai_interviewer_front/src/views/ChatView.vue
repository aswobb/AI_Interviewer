  ...
  methods: {
    ...
    async generateDownloadLink(){
      ...
      try {
        ...
        {
          message: chatBody + ",{\"role\":\"user\", \"content\":\"CSVインポートしたいので面接での質問と回答と評価を出力してください。必ず「質問」,「回答」,「各プログラム評価」,「総合評価」,「評価理由」,「改善点」の6列にしてください。評価理由:４行程度で出力してください。 改善点:４行程度で出力してください。 CSV以外の出力はしないでください。CSVデータとそうではないところがわかるように、CSVデータは```で囲ってください。\" }"
        },
        {
          headers: {
            "Content-Type": "application/json",
